...
implementation_tasks:
  ...
  phase_b:
    ...
    1. **沙盒测试完整性**：
       - 已在沙盒环境下完成了支付流程的全面测试
       - 沙盒环境支付也能正常触发会员状态更新
       - 沙盒环境与生产环境的主要区别在于交易是模拟的，但流程和API调用是完全一致的
    
    2. **沙盒与生产环境的会员状态更新机制**：
       - 支付成功页面(success.html)会通过chrome.runtime.sendMessage向扩展发送消息
       - 消息包含checkoutId和planType，用于更新会员状态
       - 扩展的background.js中handlePaymentSuccess函数处理这些消息
       - 会员状态更新逻辑对沙盒和生产环境是相同的
    ...
    4. **重要安全性问题发现**：
       - 目前支付与用户认证未关联，存在设备更换或浏览器重装后会员权益丢失风险
       - 通过本地存储修改可能伪造会员身份
       - 多设备间无法同步会员状态
       - 行业最佳实践是将支付与用户账户关联，而非设备/浏览器
       - 参见B8任务：官网认证系统集成

    task_b8: # 官网认证系统集成
      description: 将现有Firebase认证系统集成到官网，实现扩展与官网的认证状态共享
      work_items:
        - 在官网集成Firebase Authentication SDK【待完成】
        - 实现登录/注册UI组件和页面【待完成】
        - 开发扩展与官网间的认证状态共享机制【待完成】
        - 修改支付流程，要求用户先登录后支付【待完成】
        - 支付成功后将订阅信息关联到用户账户【待完成】
        - 实现会员中心页面，显示订阅状态和管理选项【后续完成】
        - 添加Auth状态变化监听，确保实时更新UI【待完成】
      
      acceptance_criteria:
        - 用户能够在官网注册和登录账户
        - 扩展和官网共享登录状态(一处登录，另一处自动登录)
        - 支付前必须先登录账户
        - 支付成功后，订阅信息与用户ID关联存储
        - 会员状态与账户绑定，而非设备/浏览器
        - 用户可在任何设备登录账户后恢复会员权益
      ...
      implementation_note: |
        此任务为新增高优先级任务，解决当前支付系统的安全性和会员权益保护问题。
        
        实施要点:
        - 复用现有Firebase项目和配置
        - 使用相同的Auth实例确保token共享
        - 在官网header添加用户区域，显示登录状态
        - 支付按钮点击时检查登录状态，未登录则引导登录
        - 支付成功后将订阅信息写入用户文档
        - 基于Firebase Auth状态实现自动登录检测
        
        技术选型:
        - 前端: React + Firebase Auth SDK
        - 数据存储: Firestore(与扩展共用)
        - 认证机制: JWT token共享
    ...
    task_b6: # 支付流程连接与成功处理
      ...
      work_items:
        - 更新UpgradeButton点击行为，连接到官网支付页面【已完成】
        - 实现ProPlanCard上升级按钮的跳转行为【已完成】
        - 创建支付成功回调页面【已完成，在B3任务中，完成支付后会通知用户回到拓展】
        - 实现会员状态验证与更新【待完成】
        - 添加支付成功后的提示和引导【待完成】
        - 实现支付完成后组件状态更新(ProBadge激活等)【待完成】
        - 支付成功后保存会员状态到用户账号，确保跨设备同步【待完成】
        - 实现账号关联的会员状态验证机制【待完成】
        - 添加从服务器获取会员状态的API【待完成】
      ...
      implementation_note: 此任务依赖于B3(官网支付页面)、B5(会员状态管理)和B8(官网认证系统)的完成,应在完成这些任务后进行。
    ...
    phase_b_verification:
      ...
      current_progress:
        ...
        - 待完成：B8(官网认证系统集成)、B6(支付流程连接)、B7(会员中心)
        - 推荐开发顺序：B8 > B6 > B7
