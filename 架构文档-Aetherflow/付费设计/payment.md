...
## 渐进式任务设计:
  implementation_tasks:
    ...
    acceptance_criteria_examples:
      scenario_1:
        name: "免费用户达到提示词存储限制"
        scenario: "免费用户尝试存储第6条提示词"
        expected: "系统显示配额限制横幅，提示用户升级"
        verification: "手动创建6条提示词，观察提示是否正确"
      
      scenario_2:
        name: "用户完成支付流程"
        scenario: "用户完成Paddle支付并返回应用"
        expected: "PRO标识亮起，升级按钮变为闪电图标，显示成功消息"
        verification: "完成测试支付流程，检查状态和UI变化"
    
    # 开发节奏管理
    development_rhythm:
      principles:
        - 小步快跑: 每个任务应较小且可独立验收
        - 持续集成: 每完成一个小功能就集成到主分支
        - 早期反馈: 实现基本功能后立即收集反馈
        - 迭代改进: 先求可用，再求完美