...
import { membershipService, MembershipState } from '../membership'; // 假设 Membership 服务在这里
import {
  QuotaLimits,
  QuotaUsage,
  QuotaType,
...
class QuotaService {
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      // 1. 获取当前会员状态以确定初始 limits
      const memberState = await this.membership.getCurrentMembership();
      this.updateLimits(memberState);
  ...
  private setupMembershipListener(): void {
    this.membership.onMembershipChange((state) => {
      safeLogger.log('[QuotaService] Membership state changed:', state.status);
      this.updateLimits(state);
      // 可能需要重新检查是否超额，或者通知UI更新
      this.notifyObservers();
    });
  }
  ...
  private resetState(): void {
    ...
    this.isInitialized = false;
    // 清空观察者？或者通知它们状态已重置？暂时先不清空
    // this.observers = [];
    safeLogger.log('[QuotaService] State reset.');
    // 通知观察者状态已重置（传递 null 或默认值）
    this.notifyObservers();
  }
